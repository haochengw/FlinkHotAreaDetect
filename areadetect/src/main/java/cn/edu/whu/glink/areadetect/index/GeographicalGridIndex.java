...
import cn.edu.whu.glink.areadetect.util.GeoUtils;
...
public class GeographicalGridIndex extends GridIndex {
  ...
  public List<Long> kRing(long index, int k) {
    List<Long> res = new ArrayList<>(4*k*k + 4*k);
    long[] coor = this.getXY(index);
    long xCenter = coor[0];
    long yCenter = coor[1];
    for (int l = 1; l <= k; l++) {
      // 第l层, 左下角为(x-l,y-l), 右上角为(x+l, y+l)
      long x = xCenter - l;
      long y = yCenter - l;
      for (int i = 0; i < 2*l + 1; i++) {
        res.add(combineXY(x + i, y));
        if (i==0) {
          continue;
        }
        res.add(combineXY(x, y + i));
      }
      x = xCenter + l;
      y = xCenter + l;
      for (int i = 0; i < 2 * l; i++) {
